...
def calc_tsne(processed_data, perplexity=20, n_components=2):
    ...
    tsne = TSNE(
        n_components=n_components, perplexity=perplexity, n_iter=500, learning_rate=100
    )
...
def show_distribution_of_clusters(clusters, dflabel, category):
    ...
    for clust in np.unique(clusters):
        ...
        if num > num_of_type:
            ...
            type_cluster = clust
        print(
            f"Cluster number {clust} has {size_of_cluster} addresses, including {num} addresses labeled as {category} (label density: {density})."
        )
    return None
